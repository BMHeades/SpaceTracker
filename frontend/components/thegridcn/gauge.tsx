...
export function Gauge({
  ...
  const [animatedPct, setAnimatedPct] = React.useState(pct)
  const prevValueRef = React.useRef(value)

  React.useEffect(() => {
    const startValue = prevValueRef.current
    const startPct = Math.max(0, Math.min(100, ((startValue - min) / (max - min)) * 100))
    ...
    >
      ...
      >
        ...
        <circle
          ...
          strokeWidth={config.stroke}
          strokeLinecap="round"
          strokeDasharray={`${(animatedPct / 100) * arcLength} ${fullCirc - (animatedPct / 100) * arcLength}`}
          transform={`rotate(${startAngleDeg} ${center} ${center})`}
